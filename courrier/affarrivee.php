...
<div align="center" class="style2">الواردات</div>
<?php
if  ($login == "ADMINSG"){ 
?>
<p align="center"><a href="arrivee.php">إضافة</a></p>
<?php
}
...
if (isset($_POST['exp'])) {$exp = (get_magic_quotes_gpc()) ? $_POST['exp'] : addslashes($_POST['exp']);}else{$exp ="";}
...
$champ1 = $_POST['champ'];
			...
			if (($codd <> "_")){
				...
				$query_resultat1 = "UPDATE arrivee SET direction = '$champ' where code = '$codd'";
...
$query0 = "SELECT expediteur FROM arrivee group by expediteur order by expediteur";
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="7" class="text">
    ...
    <tr>
    	...
    	<td ><select name="exp" size="1" id="article" tabindex="9">
        <option><?php echo $exp;?> </option>
        <option> </option>
            <?php do { 
                                     $res=$row0['expediteur'];
...
<?php
$query1 = "delete from arriveet";
$resultat1 = mysql_query($query1, $connexion) or die(mysql_error());
$dat = date('Y');
if ($du == ""){
   
 if (($login == "Technique")or($login == "Finance")){
   $query1 = "insert into arriveet select * from arrivee where annee = '$dat' and expediteur like '%$exp' and objet like '%$obj' and direction like '%$login%'";}else{
   $query1 = "insert into arriveet select * from arrivee where annee = '$dat' and expediteur like '%$exp' and objet like '%$obj'";}}
else{
 if (($login == "Technique")or($login == "Finance")){
   $query1 = "insert into arriveet select * from arrivee where expediteur like '%$exp' and objet like '%$obj' and date >= '$du' and date <= '$au' and direction like '%$login%'";}else{
   $query1 = "insert into arriveet select * from arrivee where expediteur like '%$exp' and objet like '%$obj' and date >= '$du' and date <= '$au'";} }
   
$resultat1 = mysql_query($query1, $connexion) or die(mysql_error()); 
$query ="SELECT * FROM arriveet order by date desc, num_arrivee desc";
$result = mysql_query($query,$connexion);
$row = mysql_fetch_assoc($result);
?>
<table border="1" width="100%" id="table1">
	<tr>
	    <td ><div align="center"><strong>الترايخ</strong></div></td>
...
?>
	<tr>
	  ...
	  <td><div align="center" ><?php echo $row['num_arrivee'];?></div></td>
	  <td><div align="center" ><?php echo $row['expediteur'];?></div></td>
	  <td><div align="center" ><?php echo $row['objet'];?></div></td>
	  <td><div align="center" ><?php echo $row['etat'];?></div></td>
	  <td><div align="center" ><?php echo $row['direction'];?></div></td>
	  <td>
<?php
if  ($login == "ADMINSG"){ 
?>
      <form name="refr" method="post" action="affarrivee.php" >
...
<?php
     ...
     <td ><a href ='./arrivee/<?php echo $row['annee'].$row['num_arrivee'].".pdf";?> ' target="new" ><img src="image/pdf.jpg" alt="" width="32" height="42"></a><a href ='moddepart.php?code<?php echo "=$row[code]"; ?>' ></a></td>
  </tr>
<?php
